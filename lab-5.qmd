...
ggplot(births14_habitgiven, aes(x= habit, y= weight))+
  geom_boxplot() +
  labs(
   title = "Infant Weight at Birth by Maternal Smoking Habit",
    x = "Maternal Smoking Habit",
...
**Comment:** The boxplot suggests that the babies born to nonsmoking mothers are typically heavier than babies born to smoking mothers. The range of the weight for the babies born to smoking mothers is also larger.
...
**Comment:** It is based on the least squares method. This means that R is calculating the line to have the least total distance between the line and each point. The distance between the line and the point is called the residual.

c\.

**Comment:** In this case the intercept is when the mother's age is 0, which in the context of the research question is not meaningful because you cannot have a mother who is 0 years old. It is important for the statistics but not for the reasearch question.

d\.

The slope is the predicted change in the birth weight of infants for each year that the hypothetical mother is older. In this case the relationship suggested is that for every year the mother is older the weight of the infant would increase by 0.01lb which is a very weak positive relationship.
...
**Comment:** The response variable is parsim, because the research is trying to find the amount of parasites shared by certain species/characteristics.
...
ggplot(parasites, aes(x = divergence_time, y = parsim)) +
  ...
  labs(
    title = "Parasite Similarity vs. Evolutionary Divergence Time",
...
d\. The plot suggests that there is a negative relationship, when the divergence time is longer the proportion of parasites shared goes down.
...
```{r}
model_parasite <- lm(parsim ~ divergence_time, data = parasites)
tidy(model_parasite)
```

$$
\hat{parsim} = 0.38 - 0.001(divergencetime)
$$

b\.

**Comment:** The slope is the change in the predicted proportion of parasites shared for every added 1 million years of divergence time. So every 1 million years that passes since two species diverged means that they share 0.1% less parasites. The intercept means that when the two species diverge they will share about 38% of their parasites, and this is meaningful in the context of the data because it is possible for species to have just diverged.
...
**Comment:** The regression line would suggest that from any time longer than 60 million years ago that the parasite similarity would be below 0, which is not possible.
...
**Comment:** The relationship looks linear and is still negative.
...
**Comment:** No it would not be useful because all of the predictor variables are in different units, so their scales are all different.
...
**Comment:** Yes it would be useful to compare them because it shows how variation in parasite similarity is explained by each variable, so now they can be compared to each other.
...
**Comment:** Life expectancy vs GDP is a positive relationship that is curved, higher GDP only increases life expectancy to a point and then begins to level off. Life expectancy vs LogGDP is also a postive relationship but but is more evenly spread and more in a line. The LogGDP would be better suited for a linear regression because it makes the relationship closer to a straight line.
...
**Comment:** In the context of this study the R\^2 value means that about 65% of the variation in the log GDP can be explained by changes in life expectancy. This means there is a fairly strong relationship between the two variables.
...
**Comment:** They all look to have positive relationships but there are differences between continents, such as, Africa having more points clustered in the lower life expectancy and lower LogGDP, Europe and North and South America being clustered more in the higher life expectancy and higher LogGDP, and Asia having the most variability.
...
**Comment:** In the original units the intercept GDP is 24.7 meaning that in Africa a life expectancy of 0 years old would predict the GDP of a country to be about \$25. In the original units the slope would be 1.081 meaning that the for each year of life expectancy added there would need to be an increase of 8.1% in the GDP.
...
**Comment:** The expected GDP would be about \$5,907.
...
**Comment:** I enjoyed the movement visual and the fact that he paused on years that were notable and pointed out countries that were notable as well. It was sometimes hard to remember which colours belonged to which continent, a legend would have helped. The background was also not the most helpful for seeing the details of the graph. The general takeaways are that the world has come a long way in bringing lots of the countries out of poverty in just 200 years and that looking at the trajectories it seems possible for all of the countries to make it to long lives while being prosperous. I liked how the presentation was given in general as he explained the data well and it was interesting visually.
